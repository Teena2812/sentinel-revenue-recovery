...
[![Architecture](https://img.shields.io/badge/Specification-ARCHITECTURE.md-indigo)](ARCHITECTURE.md)
...
Tested across N=80 benchmark cases (30 Failed Payments, 50 B2B Receivables):

* **Full-Batch Recovery Rate**: **30.0% (24/80 cases)** across all inbound failure volume.
* **Addressable Recovery Rate**: **35.8% (24/67 cases)** — strictly evaluating addressable cases after excluding 13 cases halted before any recovery attempt by statutory fraud, dispute, or attempt-cap rules.
* **Statutory Compliance Violations**: **0 violations** (100% compliant) vs. **11 severe violations** generated by the naive baseline.
* **Capital Recovered**: **₹9,092,801.66** (₹154.1K payments + ₹8.94M commercial B2B invoices) recovered cleanly without debtor harassment.
* **100% Locked & Reproducible**: Fully deterministic verification anchored to `SIMULATED_CURRENT_TIME = 2026-08-24 12:00:00 IST` with isolated random generator streams (`seed=42`).
...
- Built-in fallback defaults in [`core/config.py`](core/config.py) ensure the system never crashes if the configuration file is temporarily unavailable.
...
Single-pass LLM prompts can hallucinate or vacillate when debtor data contains ambiguous signals. Sentinel implements an architectural multi-sample consensus mechanism ([`agents/diagnosis.py`](agents/diagnosis.py)):
...
Production reliability is demonstrated by how safely a system fails under stress. Sentinel is verified against five induced failure modes in [`tests/test_failure_modes.py`](tests/test_failure_modes.py):
...
- Anchored all simulated temporal checks to a fixed simulation constant: `config.SIMULATED_CURRENT_TIME = datetime(2026, 8, 24, 12, 0, 0) IST`.
...
- This guaranteed 100% byte-identical benchmark reproducibility across all operating systems and test runners.

### Self-Correction Case Study 2: Calibration Methodology & Pre-Pipeline Exclusions
In an earlier calibration audit, 19 cases appeared in the lowest confidence bucket (`0.0–0.2`) with a 0% recovery rate. When auditing the underlying data, we discovered that these 19 cases were pre-pipeline statutory holds (fraud stops, dispute holds, attempt-cap ceilings, active promises) where `diagnose()` had never executed. They had been assigned a placeholder `confidence = 0.0`.

**The Architectural Fix**:
- Rebuilt [`scripts/confidence_calibration.py`](scripts/confidence_calibration.py) to filter out non-diagnosed cases (`is_diagnosed_case`), isolating strictly the **61 cases that genuinely ran through `diagnose()`**.
...
Evaluated via [`scripts/confidence_calibration.py`](scripts/confidence_calibration.py) across all 61 cases that reached `diagnose()`:
