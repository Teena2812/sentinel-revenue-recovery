...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .badge-approved-gate { background: rgba(16, 185, 129, 0.15); color: #34D399; font-size: 11px; padding: 3px 8px; border-radius: 4px; }
    .badge-blocked-gate { background: rgba(239, 68, 68, 0.2); color: #F87171; font-size: 11px; padding: 3px 8px; border-radius: 4px; font-weight: 700; }
...
<body>
  <div class="container">
    ...
    <section class="kpi-grid">
      ...
      <div class="kpi-card" style="--kpi-color: #F87171;">
        <div class="kpi-label">Gate Intercepts</div>
        <div class="kpi-value" id="kpi-gate-blocked">--</div>
        <div class="kpi-subtext">Fraud &amp; attempt-cap stops</div>
      </div>
    ...
    <div class="controls-card">
      ...
      <div class="filter-tabs" id="filterTabs">
        ...
        <button class="tab-btn" data-filter="GATE_BLOCKED">Gate Intercepts (<span id="count-blocked">0</span>)</button>
  ...
  <script>
    ...
    function computeAndRenderKPIs() {
      ...
      const gateBlocked = AUDIT_DATA.filter(c =>
      ...
      document.getElementById('kpi-gate-blocked').textContent = gateBlocked.length;
      ...
      document.getElementById('count-blocked').textContent = gateBlocked.length;
    ...
    function renderTable() {
      ...
      const filtered = AUDIT_DATA.filter(c => {
        ...
        if (currentFilter === 'GATE_BLOCKED' && !(c.status === 'GATE_BLOCKED' || c.status === 'STOPPED' || (c.gate_violations && c.gate_violations.length > 0))) return false;
      ...
      filtered.forEach(c => {
        ...
        let statusSymbol = '●';
        if (c.status === 'RECOVERED') { statusBadgeClass = 'badge-recovered'; statusSymbol = '●'; }
        else if (c.status === 'ESCALATED') { statusBadgeClass = 'badge-escalated'; statusSymbol = '▲'; }
        else if (c.status === 'GATE_BLOCKED') { statusBadgeClass = 'badge-blocked'; statusSymbol = '✖'; }
        else if (c.status === 'STOPPED') { statusBadgeClass = 'badge-stopped'; statusSymbol = '■'; }

        // Gate Badge Style
        const gateApproved = c.gate_approved;
        const gateBadgeClass = gateApproved ? 'badge-approved-gate' : 'badge-blocked-gate';
        const gateText = gateApproved ? '✓ APPROVED' : '⚠ BLOCKED';
        ...
        tr.innerHTML = `
          ...
          <td>
            <span class="${gateBadgeClass}">${gateText}</span>
            ${!gateApproved && c.gate_violations.length > 0 ? `<div style="font-size: 10px; color: #F87171; max-width: 200px; margin-top: 3px;">${c.gate_violations[0].substring(0, 45)}...</div>` : ''}
          </td>
          <td><span class="badge ${statusBadgeClass}">${statusSymbol} ${c.status}</span></td>
